...
// 	},
	...
	pointCoordsCWToTdString(ptC, ptW){
		...
		const strW = `<td><span>(${Math.round(ptW.x)}, ${Math.round(ptW.y)})</span></td>`;
		return strC + strW;
	},
	pointCoordsCWLabToTdString(ptC, ptW, lab){
		const strL = `<td><span>${lab}</span></td>`;
		const strC = `<td><span>(${Math.round(ptC.x)}, ${Math.round(ptC.y)})</span></td>`;
		const strW = (typeof ptW.x !== 'number') ? '<td><span>-</span></td>' : `<td><span>(${Math.round(ptW.x)}, ${Math.round(ptW.y)})</span></td>`;
